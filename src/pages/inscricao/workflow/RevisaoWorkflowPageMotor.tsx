...
/**
 * Página Revisão para Workflow Engine (Motor BPMN)
 *
 * Cópia EXATA da RevisaoPage.tsx original com adaptações mínimas:
 * - Usa APENAS useInscricaoStore (remove useInscricaoContext)
 * - handleSubmit() chama completeStep() do workflow engine ao final
 * - Mantém 100% do layout e funcionalidades aprovadas em produção
 *
 * Layout validado pelo usuário e já em produção
 */
export default function RevisaoWorkflowPageMotor() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        const blockchainResult = await sendToBlockchain(jsonString, String(processoId));
      ...
      console.log('Completando step final no workflow:', {
      ...
      console.log('Workflow finalizado:', response);
