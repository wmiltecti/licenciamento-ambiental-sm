...
interface ReferenceUnitViewProps {
  item: any;
  onBack: () => void;
}

export default function ReferenceUnitView({ item, onBack }: ReferenceUnitViewProps) {
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-100">Unidade de Referência</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {item.abbreviation && (
            <div>
              <label className="text-sm font-medium text-gray-500 block mb-2">Sigla</label>
              <p className="text-lg text-gray-900 font-mono font-semibold">{item.abbreviation}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-200">
            ...
            {item.abbreviation && (
              <div>
                ...
                <p className="text-sm text-gray-900 mt-1 font-semibold font-mono">{item.abbreviation}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-semibold text-blue-900 mb-1">
              Sobre as Unidades de Referência
            </h4>
            <p className="text-sm text-blue-800">
              As unidades de referência são utilizadas para padronizar medidas e quantificações
              no processo de licenciamento ambiental. Elas servem como base para cálculos de
              taxas, emissões, consumos e outras métricas importantes.
            </p>
          </div>
        </div>
      </div>
